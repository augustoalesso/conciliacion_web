...
# Mapeo robusto para aceptar variantes de nombres de columnas
COLUMNAS_MAPEO = {
    'Fecha': 'Fecha', 
    'Debe': 'Debe', 
    'Haber': 'Haber',
...
def formatear_reporte(df, estado, suf_c='_C', suf_b='_B'):
    if df.empty:
        return pd.DataFrame(columns=get_columnas_finales())
    df_rep = df.copy()
    df_rep['Estado'] = estado
    # Manejo seguro de columnas de fecha con sufijos
    col_fecha_b = f'Fecha{suf_b}'
    col_fecha_c = f'Fecha{suf_c}'
    df_rep['Fecha'] = df_rep[col_fecha_b].fillna(df_rep[col_fecha_c]) if col_fecha_b in df_rep and col_fecha_c in df_rep else df_rep.get('Fecha')
...
def ejecutar_conciliacion(df_c, df_b):
        ...
        for _, rc in df_c_p2.iterrows():
            ...
            if not matches.empty:
                ...
                t_list.append({
                    ...
                    'Fecha': rb['Fecha'], 
                    'Monto_C': rc['Monto'], 'Monto_B': rb['Monto'], 
                    'Concepto_C': rc['Concepto'], 'Concepto_B': rb['Concepto'], 
                    f'{ID_COL}_C': rc[ID_COL], f'{ID_COL}_B': rb[ID_COL]
                })
    ...
    return final.sort_values('Fecha', na_position='last')
        ...
        if up_c and up_b:
            with st.spinner("Conciliando registros..."):
                ...
                if dc is not None and db is not None:
                    ...
                    st.metric("Movimientos Procesados", len(resultado))
                    
                    excel_file = to_excel_premium(resultado)
                    st.download_button(
                        label="⬇️ Descargar Reporte FinMatch (Excel)",
                        data=excel_file,
